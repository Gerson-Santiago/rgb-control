...
class MainWindow(Adw.ApplicationWindow):
    def __init__(self, application):
        ...
        logger.info("Carregando janela principal...")
    ...
    def on_service_toggle(self, switch, state):
        logger.info(f"Comando de Serviço: {'LIGAR' if state else 'DESLIGAR'}")
        success = self.backend.set_service_state(state)
        if not success:
            logger.warning("Falha ao alterar estado do serviço. Revertendo toggle.")
            # Reverte estado se falhar ao usar o pkexec
            return True
        return False

    def on_mode_toggle(self, switch, state):
        logger.info(f"Comando de Modo LED: {'ATIVAR' if state else 'DESATIVAR'}")
